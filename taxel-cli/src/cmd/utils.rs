...
pub fn log_response(log_path: &Path, response: &EricResponse) -> Result<(), anyhow::Error> {
    ...
    if !response.payload.validation_response.is_empty() {
        println!(
    ...
    if !response.payload.server_response.is_empty() {
        println!(
    ...
    validation_response_file.write_all(response.payload.validation_response.as_bytes())?;
    server_response_file.write_all(response.payload.server_response.as_bytes())?;
