use crate::{
    ebilanz::{BASELINE_ROLE_URIS, REPORT_ELEMENT_TO_ROLE_URI, REQUIRED_NIL_TUPLE_CHILDREN},
    report::{FactValue, ReportSection},
};
use anyhow::Context;
...
/// [`Report::populate`](crate::Report::populate).
...
///
/// This exists because [`ReportSection::recompute_calculated_values`] only
/// fixes the in-memory `FactRow` values shown in the table — it doesn't
/// itself persist into `InstanceDocument`. Persistence for a live edit
/// happens through the normal snapshot-diff path used by the GUI's Save
/// flow; this function is for the one path (rebuilding the instance from
/// scratch) that persists to `InstanceDocument` directly, bypassing that
/// diff.
